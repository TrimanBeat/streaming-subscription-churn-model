...
TRAIN_MODEL_READY_PATH = Path("data/processed/train_model_ready.csv")
...
def train_model_ready_csv(train_model_ready: pd.DataFrame) -> str:
    ...
    TRAIN_MODEL_READY_PATH.parent.mkdir(parents=True, exist_ok=True)
    train_model_ready.to_csv(TRAIN_MODEL_READY_PATH, index=False)
    return str(TRAIN_MODEL_READY_PATH)
...
def incoming_prediction_ready(
    ...
    """Pasa incoming_customers por el mismo pipeline de preparación que train y lo alinea al schema del train."""
...
def segment_summary_r_csv() -> str:
    """Genera un resumen por segmentos usando R a partir del dataset train preparado."""
    input_path = TRAIN_MODEL_READY_PATH
...
@dg.asset(deps=[train_model_ready_csv])
def combined_training_data(train_model_ready: pd.DataFrame, new_customers_for_training: pd.DataFrame) -> pd.DataFrame:
    """Combina el dataset histórico preparado con nuevos clientes seleccionados para reentrenamiento."""
    base_df = train_model_ready.copy()
